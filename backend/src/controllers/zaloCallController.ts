...
import { io } from "../server"; // đảm bảo server.ts export io
...
export const createCallController = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log("📦 userId:", userId, "| telesale:", telesale?._id || "system");
    ...
    console.log("📞 Gọi API Zalo...");
    const zaloRes = await axios.post(
      "https://openapi.zalo.me/v3.0/oa/call/outbound",
      { user_id: userId, call_type: "audio" },
      {
        headers: {
          access_token: token,
          "Content-Type": "application/json",
        },
        timeout: 10000, // tránh treo vô hạn
      }
    );
