...
export default function Header() {
  ...
  return (
    <div
      className="flex flex-col w-full px-4 overflow-hidden bg-right-top bg-no-repeat bg-primary text-primaryForeground pt-st"
      style={{
        backgroundImage: `linear-gradient(90deg, rgba(176, 188, 216, 0.6) 0%, rgba(0, 0, 0, 0.6) 100%), url(${headerIllus})`,
      }}
    >
        ...
        {handle?.logo ? (
          <>
            <img
              ...
              className="flex-none w-8 h-8 border-b-2 border-white rounded-full"
            />
          ...
          <>
            {showBack && (
              <div className="px-2 py-1 cursor-pointer" onClick={() => navigate(-1)}>
      ...
      {handle?.search && (
        <div className="flex w-full py-2 space-x-2">
          <SearchBar
            onFocus={() => {
          ...
          <TransitionLink to="/profile">
            {userInfo.state === 'hasData' && userInfo.data ? (
              <img className="w-8 h-8 rounded-full" src={userInfo.data.avatar} />
            ) : (
              <DefaultUserAvatar
                width={32}
                height={32}
