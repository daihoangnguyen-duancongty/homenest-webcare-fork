...
//   requestWithFallback<string[]>('./../public/1712119139749 (1).png', [])
// );
...
export const stationsState = atom(async () => {
  ...
  const stationsWithDistance = stations.map((station) => ({
    ...
    distance: location
      ? formatDistant(
          calculateDistance(location.lat, location.lng, station.location.lat, station.location.lng)
        )
      : undefined,
  }));
...
export const shippingAddressState = atomWithStorage<ShippingAddress | undefined>(
  CONFIG.STORAGE_KEYS.SHIPPING_ADDRESS,
  undefined
);
...
export const ordersState = atomFamily((status: OrderStatus) =>
  atomWithRefresh(async () => {
    ...
    const clientSideFilteredData = allMockOrders.filter((order) => order.status === status);
